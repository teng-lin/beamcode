...
describe("E2E Real SDK-URL SessionCoordinator", () => {
  ...
  afterEach(async (context: TestContextLike) => {
    dumpTraceOnFailure(context, activeCoordinators, "claude-e2e-debug");
  ...
  it.runIf(runSmoke)("launch emits process spawn and records PID", async () => {
    const { coordinator, sessionId } = await setupRealSession("claude");
    activeCoordinators.push(coordinator);

    await waitForCoordinatorEvent(
      coordinator,
      "process:spawned",
      sessionId,
      () => typeof coordinator.launcher.getSession(sessionId)?.pid === "number",
      15_000,
    );
  ...
  it.runIf(runSmoke)(
    ...
    async () => {
      const { coordinator, sessionId, port } = await setupRealSession("claude");
      activeCoordinators.push(coordinator);
      await waitForBackendConnectedOrExit(coordinator, sessionId, 20_000);
  ...
  it.runIf(runSmoke)("relaunch reconnects backend and broadcasts cli_connected again", async () => {
    ...
    try {
      ...
      const relaunched = await coordinator.launcher.relaunch(sessionId);
  ...
  it.runIf(runSmoke)("invalid cwd exits session without backend connection", async () => {
    const { coordinator } = await setupRealSession("claude");
    activeCoordinators.push(coordinator);
  ...
  it.runIf(runSmoke)("watchdog relaunches stale starting session", async () => {
    ...
    const originalGetStarting = launcherAny.getStartingSessions.bind(coordinator.launcher);
    const originalRelaunch = launcherAny.relaunch.bind(coordinator.launcher);
    let relaunchCount = 0;
    ...
    try {
      const managerAny = coordinator as unknown as { startReconnectWatchdog: () => void };
  ...
  it.runIf(runFull)(
    "resume across coordinator restart (storage-backed) preserves conversation context",
    async () => {
      ...
      let manager1: SessionCoordinator | null = null;
      let manager2: SessionCoordinator | null = null;
      try {
        ...
        manager1 = new SessionCoordinator({
          config: config1,
          storage: fileStorage,
          server: server1,
          adapter: new ClaudeAdapter(),
        ...
        activeCoordinators.push(manager1);
        ...
        // Second coordinator: relaunch and verify the model remembers
        ...
        manager2 = new SessionCoordinator({
          config: config2,
          storage: fileStorage,
          server: server2,
          adapter: new ClaudeAdapter(),
        ...
        activeCoordinators.push(manager2);
